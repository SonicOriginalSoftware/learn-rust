...
The purpose of this example is to showcase how to use `Trait`s and `Generics` to take advantage of `Rust`s monomorphisation in the compiler.

_Monomorphisation_ is the process the compiler uses to inline your code using statically-defined relationships between your `Trait`s and `Generics`.

_Static dispatch_ is the runtime behavior of using these optimized, inlined functions as the appropriate instance method for the target object.

Without _monomorphisation_ there would be runtime overhead of processing the `vtable` to perform a pointer lookup in order to find what the target object is to call the method upon.

_Dynamic dispatch_ is addressed [here](../learning-dynamic-dispatch).

# Explanation

We want to be able to create an object that can be passed around with a known ability; it `CanDoThings`. So we create a `Trait` that declares a behavior `do_it` (the _thing_, obviously).
